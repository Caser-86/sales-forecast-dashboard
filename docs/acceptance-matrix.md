...
| AC-020 | Replenishment | Missing or stale inventory inputs block a suggestion and identify the missing input | Domain/API test | Partial: import validation, missing-field validation, and stale snapshot 503 pass; missing forecast/snapshot combination still needs an explicit per-item unavailable reason |
