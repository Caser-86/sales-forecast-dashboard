...
| AC-011 | ABC | ABC is presented as demand priority and is not used alone as stockout risk | Schema/UI copy test | Partial: UI copy and active-snapshot formula pass; legacy demo fallback still derives risk from ABC until generated inventory becomes mandatory |
...
| AC-019 | Replenishment | Suggested quantity follows net available, lead time, review period, safety stock, pack size, and MOQ | `tests/test_replenishment.py` hand-calculated fixtures | Partial: formula, active-snapshot integration, and inventory breakdown fields pass; saved-plan/UI explanation flow remains |
...
| AC-022 | Plans | A replenishment draft can be saved, reopened after restart, and exported with immutable version metadata | `tests/test_plans.py` and restore run | Partial: SQLite snapshot, restart read, version metadata, CSV export, backup restore, and frontend save/export entry pass; browser E2E remains |
