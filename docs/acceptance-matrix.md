...
| AC-022 | Plans | A replenishment draft can be saved, reopened after restart, and exported with immutable version metadata | `tests/test_plans.py` and restore run | Partial: SQLite snapshot, restart read, version metadata, CSV export, and backup restore tests pass; browser flow remains |
...
| AC-025 | Failure semantics | Partial prediction includes requested/succeeded/failed counts and prevents saving an incomplete draft | Service/API/UI tests | Partial: coverage/UI warning and API save gate pass; browser save flow remains |
