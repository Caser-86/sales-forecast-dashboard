...
- 下一目标为本地完整演示版 V2，M1/T1、M1/T2 共享壳、T3/T4/T5 第一切片、T6 预测分析第一切片和 T7 库存试算第一切片已完成，详细路线见 [`docs/local-demo-plan.md`](docs/local-demo-plan.md)。
...
当前处于本地完整演示版 V2 执行阶段。M1/T1、M1/T2 共享壳、T3/T4/T5 第一切片、T6 预测分析第一切片和 T7 库存试算第一切片已完成：现已支持 CSV 模板、大小/行数/字段预检、行级错误报告下载、候选版本清单、单指针运行快照发布/详情/回滚、候选模型与训练任务管理、商品/门店预测明细和服务端补货试算；请求级一致性、批量人工调整和后续业务页面仍按路线图实现。详细范围、任务依赖、拟修改文件和验收标准见 [`docs/local-demo-plan.md`](docs/local-demo-plan.md)，未完成项索引见 `TODO.md`。
...
2. 完成 T4 切换中请求的一致快照验收，再进入 T6 完整口径验收、T7 批量调整和 T8 审批页面。
