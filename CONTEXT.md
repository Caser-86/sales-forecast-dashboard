...
- 当前没有正在进行的 V1 业务功能开发；未完成内容均属于生产化后续任务，见 `TODO.md`。
...
当前没有已承诺的 V1 功能处于开发中。生产化方向按优先级记录在 `TODO.md`，在明确业务数据、部署环境和验收指标前不应擅自扩展范围。

## 未完成任务

见 [`TODO.md`](TODO.md)。其中的 P1/P2 项目不阻塞当前面试版 V1，除非未来把目标改为生产部署。
...
1. 先明确真实销售/库存数据契约和接入方式，再实现生产数据导入与新鲜度监控。
2. 建立训练、模型发布、回滚和漂移告警的运营闭环。
3. 根据部署环境补齐多用户权限、密钥托管、队列化推理和可观测性。
