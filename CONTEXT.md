...
- 下一目标为本地完整演示版 V2，M1/T1、M1/T2 共享壳、T3 任务后端第一切片和 T4 数据中心第一切片已完成，详细路线见 [`docs/local-demo-plan.md`](docs/local-demo-plan.md)。
...
当前处于本地完整演示版 V2 执行阶段。M1/T1、M1/T2 共享壳、T3 任务后端第一切片和 T4 数据中心第一切片已完成：现已支持 CSV 模板、大小/行数/字段预检、行级错误、候选版本清单和单指针运行快照发布/激活；完整模型中心、候选回滚、请求级一致性和后续业务页面仍按路线图实现。详细范围、任务依赖、拟修改文件和验收标准见 [`docs/local-demo-plan.md`](docs/local-demo-plan.md)，未完成项索引见 `TODO.md`。
...
1. 完成 T3 训练期间总览并行服务、模型中心和完整失败恢复 E2E。
2. 完成 T4 的候选发布/回滚失败边界与重启后快照一致性，再进入 T5–T8 的模型、预测、补货和审批页面。
