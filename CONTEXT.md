...
- 下一目标为本地完整演示版 V2，M1/T1、M1/T2、T3/T4/T5、T6 预测分析、T7 库存试算、T8 本地角色审批和 T9 场景/恢复已完成；T3/T5 已增加真实训练失败/重试/候选激活、完整训练跨页面一致性和训练期间 API 稳定性浏览器回归，T10 已增加进程级离线冒烟和鉴权完整业务回放；固定 4 核/8GB 参考机与完全断网人工复演仍待验收，详细路线和证据见 [`docs/local-demo-plan.md`](docs/local-demo-plan.md) 与 [`docs/local-demo-v2-acceptance-2026-09-22.md`](docs/local-demo-v2-acceptance-2026-09-22.md)。
...
当前处于本地完整演示版 V2 执行阶段。M1/T1、M1/T2、T3/T4/T5、T6、T7、T8 和 T9 已完成：现已支持 CSV 模板、大小/行数/字段预检、行级错误报告下载、候选版本清单、单指针运行快照发布/详情/回滚、候选模型与训练任务管理、商品/门店预测明细、服务端补货试算、草案历史/审批/导出、版本质量诊断、确定性场景、备份恢复和脱敏诊断；训练失败/重试、完整训练跨页面版本一致性、训练期间服务稳定性和鉴权完整业务回放均有真实浏览器证据。固定参考机性能、完全断网复演和最终现场收尾仍按 T10 验收。详细范围、任务依赖、拟修改文件和验收标准见 [`docs/local-demo-plan.md`](docs/local-demo-plan.md)，未完成项索引见 `TODO.md`。
...
1. 在固定参考机完成容量、长期内存和完全断网复演。
2. 完成 T10 最终业务链路验收，再评估生产化需求。
