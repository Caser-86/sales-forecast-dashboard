...
- 下一目标为本地完整演示版 V2，M1/T1、M1/T2 共享壳、T3/T4/T5 第一切片、T6 预测分析第一切片、T7 库存试算第一切片、T8 本地角色审批第一切片和 T9 场景/恢复第一切片已完成；T3/T5 已增加非生产 smoke 档案的真实训练失败/重试/候选激活浏览器回归，详细路线和证据见 [`docs/local-demo-plan.md`](docs/local-demo-plan.md) 与 [`docs/local-demo-v2-acceptance-2026-09-22.md`](docs/local-demo-v2-acceptance-2026-09-22.md)。
...
- 本地角色审批、确定性场景、SQLite 备份恢复、脱敏诊断包和离线资源检查。
- Python 测试、Ruff、前端语法检查、Compose 校验、镜像启动、Chromium E2E 和依赖审计门禁。

## 当前正在开发

当前处于本地完整演示版 V2 执行阶段。M1/T1、M1/T2 共享壳、T3/T4/T5 第一切片、T6 预测分析第一切片、T7 库存试算第一切片、T8 本地角色审批第一切片和 T9 场景/恢复第一切片已完成：现已支持 CSV 模板、大小/行数/字段预检、行级错误报告下载、候选版本清单、单指针运行快照发布/详情/回滚、候选模型与训练任务管理、商品/门店预测明细、服务端补货试算、草案历史/审批/导出、版本质量诊断、确定性场景、备份恢复和脱敏诊断；另有真实 worker 失败/重试/候选激活的非生产 smoke 浏览器回归。固定参考机性能、完全断网复演和最终全链路仍按 T10 验收。详细范围、任务依赖、拟修改文件和验收标准见 [`docs/local-demo-plan.md`](docs/local-demo-plan.md)，未完成项索引见 `TODO.md`。
...
- 本地 `-WithAuth` 提供演示角色会话和计划审批；仍不是生产级多租户身份、RBAC 或密钥托管。
...
1. 补齐 T3/T5 普通完整训练失败恢复、模型发布/回滚和跨页面版本一致性 E2E。
2. 在固定参考机完成容量、长期内存和完全断网复演。
3. 完成 T10 最终业务链路验收，再评估生产化需求。
